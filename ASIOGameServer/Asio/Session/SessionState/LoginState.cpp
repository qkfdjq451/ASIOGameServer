...
void LoginState::On_Read(const PS & symbol, void * recv_data, unsigned short size)
{
	...
	case PS::REQ_DUMMIY_CHARACTER:
	{
		...
		if (ss)
		{
			...
			shared_ptr<int> currentCode = make_shared<int>(--charcode);
			string query;
				...
				[ss, currentCode](string str, bool result)
			{
				...
				if (result)
				{
					...
					//케릭터 정보 조회하기
					auto func = std::make_shared<Function<void>>([ss, currentCode]
					{
						auto user = ss->GetUser().lock();
						if (user)
						{
							//Character 객체 만들기
							auto character = user->LoadCharacterInfo(*currentCode);
							...
							//Exp부분
							{
								auto fbb = character->Make_Fbb_Exp();								
								ss->PushSend(PS::CHANGE_EXP, move(fbb));
							}
